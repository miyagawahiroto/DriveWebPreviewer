...
### 実装方針
- **権限最小化（プライバシーファースト）**: OAuth トークン・ファイル内容・ファイル ID を外部サーバーへ送信しない。すべてブラウザ内（Service Worker / `chrome.identity` / `chrome.storage`）で完結させる。`manifest.json` の `permissions` / `host_permissions` は必要最小限に絞る
- **【厳守】Drive データは拡張内部で完結させる（外部経由を禁止）**: Drive ファイルの取得・描画は **background の Service Worker（`lib/drive-api.ts`）→ `chrome-extension://` のプレビュータブ**で行い、**外部のページ・サーバーを一切経由させない**。通信先は Google API（`*.googleapis.com` / `oauth2.googleapis.com`）のみ。外部の静的ホスティング等にファイル・トークン・ID を渡す実装は禁止
  - 補足: 一般公開向けに `drive.file` ＋ Google Picker（外部ホスト）方式を一時検討したが、**外部ページにトークンを渡さない方針を優先して採用を見送り**、`drive.readonly` ＋ content-script の「Web プレビュー」1 クリック方式を採用した（経緯は `docs/PICKER.md`）。一般公開時は restricted スコープのため CASA が必要
...
npm run build      # 本番ビルド（dist/ 出力。esbuild）
npm run typecheck  # 型チェック（tsc --noEmit）
# npm run lint     # ESLint は未導入（フォローアップ）
```
...
| `docs/SETUP_OAUTH.md` | 実 Drive をローカル確認するための OAuth 設定手順（開発者の一度きり） |
| `docs/DISTRIBUTION.md` | 配布フェーズ（社内→一般公開）・OAuth スコープ・Google 審査の方針 |
| `docs/PUBLISHING.md` | 公開ロードマップ（Chrome ウェブストア＋OAuth 検証・必要素材・手順） |
| `docs/PICKER.md` | drive.file ＋ Google Picker 方式（一般公開向け・審査軽減）の設計 |
| `docs/CASA.md` | 一般公開で `drive.readonly` を通すための年次セキュリティ評価（CASA）の申請・通過・更新手順 |
| `docs/VERIFICATION_ASSETS.md` | OAuth 検証 / CASA 提出用の素材（スコープ利用目的の説明文・デモ動画台本・添付 URL 一覧） |
| `docs/PRIVACY.md` | プライバシーポリシー（公開して URL 化。OAuth 検証・CASA の必須要件） |
| `docs/PATH_RESOLUTION.md` | 相対パス → 親フォルダ内ファイル ID の解決アルゴリズム |
| `docs/CACHE.md` | Cache API のキー設計・無効化・上限つき eviction・セッション分離 |
| `docs/CONTENT_TYPE.md` | 拡張子 / mimeType → Content-Type マッピング |
| `docs/MERMAID.md` | Markdown 内 Mermaid 記法のプレビュー対応（クライアント側描画・CSP・スクリプト配信） |
| `docs/PERFORMANCE.md` | メモリ/CPU 最適化（大ファイルのストリーミング透過・Range・キャッシュ上限・パス解決メモ化・重複排除） |
| `docs/UI.md` | コンテンツスクリプトのボタン注入・ポップアップ・設定ページ |
| `docs/DEMO.md` | OAuth 不要のローカル動作確認（デモモード） |
| `docs/SECURITY.md` | 権限最小化・トークン保護・プライバシー方針・拡張ページ CSP の制約 |
| `docs/SANDBOX_PREVIEW.md` | 自己完結 HTML をサンドボックス表示しインライン JS を動かす出し分け設計 |
