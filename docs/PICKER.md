...
| **API キー** | Google Picker の `setDeveloperKey` | 新規作成。Picker API（Google Picker API）を有効化し、キーを発行。`manifest` には入れずビルド時に注入する想定（公開拡張では完全な秘匿は不可なため、キーは Picker/Drive に用途制限する） |
...
- API キーは公開拡張では完全秘匿できないため、用途・参照元制限を必ず設定する
